...
for num_features in range(34, 45):
    ...
    # leaveone-out kfold of 100 splits for feature selection
    ...
    for train_index, test_index in tqdm(kf.split(patient_ids)):
        ...
        for (train_index, test_index) in sgkf.split(x_train, y_train, train_groups):
            ...
            mi = mutual_info_classif(norm_fold_x_train, fold_y_train, random_state=42, n_jobs=-1)
    ...
    indexes_filename = 'micbest_indexes'+database.split('.')[0]+'_'+str(num_features)+'features.txt'
    ...
    results_filename = 'micresults'+database.split('.')[0]+'svmminmax'+str(num_features)+'features.txt'
    ...
    roc_filename = 'microc'+database.split('.')[0]+'svmminmax'+str(num_features)+'features.png'
    rocch_filename = 'microcch'+database.split('.')[0]+'svmminmax'+str(num_features)+'features.png'
    pr_filename = 'micpr'+database.split('.')[0]+'svmminmax'+str(num_features)+'features.png'
    det_filename = 'micdet'+database.split('.')[0]+'svmminmax'+str(num_features)+'features.png'
    detch_filename = 'micdetch'+database.split('.')[0]+'svmminmax'+str(num_features)+'features.png'
    score_filename = 'micvscore'+database.split('.')[0]+'svmminmax'+str(num_features)+'features.png'
    # plot the roc curve
