...
def run_feature_selection(num_features):
    ...
    # Calcular el número de repeticiones de cada valor en la columna "patient"
    patient_counts = df_input["patient"].value_counts()

    num_sp = patient_counts.iloc[0]  # Esto selecciona el número de repeticiones para el primer paciente en la lista

    # vectores para almacenar las metricas de evaluación de los distintos smooth pursuits
    tps = np.zeros(num_sp)
    tns = np.zeros(num_sp)
    fps = np.zeros(num_sp)
    fns = np.zeros(num_sp)
    corrects = np.zeros(num_sp)
    positives = np.zeros(num_sp)
    negatives = np.zeros(num_sp)
    ...
    for train_index, test_index in kf.split(patient_ids): #tqdm(kf.split(patient_ids)):
        ...
        # Calcular las diferencias entre las predicciones de clase 1 y clase 0 en una sola operación
        differences = y_pred[:, 1] - y_pred[:, 0]
        # Comparar las diferencias con el umbral para obtener un array binario
        binary_predictions = (differences > best_threshold).astype(int)
        ...
        # Iterar sobre cada posición en el vector binary_predictions
        for i in range(num_sp):
            # True Positive (TP): la predicción es 1 y la respuesta es 1
            if binary_predictions[i] == 1 and answer == 1:
                tps[i] += 1
            # False Positive (FP): la predicción es 1 y la respuesta es 0
            elif binary_predictions[i] == 1 and answer == 0:
                fps[i] += 1
            # True Negative (TN): la predicción es 0 y la respuesta es 0
            elif binary_predictions[i] == 0 and answer == 0:
                tns[i] += 1
            # False Negative (FN): la predicción es 0 y la respuesta es 1
            elif binary_predictions[i] == 0 and answer == 1:
                fns[i] += 1
    ...
    accuracies = corrects / (positives + negatives)
    ...
    print("accuracies: " + accuracies_str)
    ...
    with open(results_filename, "a") as f:
        ...
        f.write("accuracies: " + accuracies_str + "\n")
...
'''
# Testing idea but it didn't work out
#remove_sp = ["SmoothPur_1", "SmoothPur_2", "SmoothPur_8", "SmoothPur_10"]
#df = df[~df['smoothp'].isin(remove_sp)]
