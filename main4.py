...
database = "database7b.csv" #"database6c.csv"
...
# yonly = ["SmoothPur_5", "SmoothPur_6", "SmoothPur_7", "SmoothPur_8"]
# # Remove rows where 'eyeaxis' is 'x' and 'smoothp' is in the list task
# df = df[~((df['eyeaxis'] == 'y1') & (df['smoothp'].isin(xonly)))]
# df = df[~((df['eyeaxis'] == 'y2') & (df['smoothp'].isin(xonly)))]
# df = df[~((df['eyeaxis'] == 'x1') & (df['smoothp'].isin(yonly)))]
# df = df[~((df['eyeaxis'] == 'x2') & (df['smoothp'].isin(yonly)))]
...
#poner los campos que faltan a la base de datos database6c.csv
if database == "database6c.csv":
    df["eyeaxis"] = "x"
    df["delay"] = 0
    df["dimension"] = 0
if database == 'database7b.csv':
    df["delay"] = 0
    df["dimension"] = 0
...
#if database == "database7.csv":
#    df = df.loc[~df['eyeaxis'].isin(['y1', 'y2'])]
...
for train_index, test_index in tqdm(kf.split(patient_ids)):
    ...
    pipeline.fit(x_train, y_train, classifier__groups=train_groups)
    #pipeline.fit(x_train, y_train, feature_selector__groups=train_groups, classifier__groups=train_groups)

    #list_features.append(pipeline.named_steps['feature_selector'].k_feature_idx_)


    # best_model = pipeline.named_steps['classifier'].best_estimator_

    y_train_pred = pipeline.predict_log_proba(x_train)
    ...
    y_pred = pipeline.predict_log_proba(x_test)
...
#print("selected features in each iteration of the leave-one-out:")
#for subset in list_features:
#    print(subset)
